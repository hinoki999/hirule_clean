...
class AdvancedRiskManager:
    ...
    def calculate_correlation_matrix(self) -> np.ndarray:
        ...
        # Find minimum length of returns across all symbols
        min_length = min(len(self.returns_history[symbol]) for symbol in symbols)

        # Create return matrix with consistent size
        returns_matrix = np.zeros((min_length, n_symbols))
        for i, symbol in enumerate(symbols):
            returns = list(self.returns_history[symbol])[-min_length:]  # Take last min_length returns
    ...
    def calculate_portfolio_var(self, positions: Dict[str, Decimal]) -> float:
        ...
        volatilities = np.array([
            np.std(list(self.returns_history[s]))
            for s in symbols
        ])
